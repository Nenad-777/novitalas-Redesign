import Header from "@/components/Header";
import Footer from "@/components/Footer";
import ImageCaption from "@/components/ImageCaption";
import ShareButton from "@/components/ShareButton";
import SeoMeta from "@/components/SeoMeta";
import { useTheme } from "@/contexts/ThemeContext";

const IMAGE_SRC = "/news/izbori-rezultati.jpg";

const TITLE =
  "Tesne razlike i smanjenje prednosti vlasti: preliminarni rezultati u deset opština";

const DECK =
  "Preliminarni rezultati lokalnih izbora održanih 29. marta u deset opština u Srbiji pokazuju da je vladajuća koalicija zadržala vlast u svim sredinama, ali uz vidljivo smanjene razlike i tesne odnose snaga u više opština, pri čemu su u pojedinim slučajevima odlučivali jedan ili dva odbornička mandata.";

interface MunicipalityData {
  name: string;
  turnout: string;
  lists: string[];
  belowThreshold: string;
  note?: string;
}

const MUNICIPALITIES: MunicipalityData[] = [
  {
    name: "Bor",
    turnout: "25.982 od 38.492 birača (67,49%)",
    lists: [
      `Lista „Aleksandar Vučić – Bor, naša porodica!" osvojila je 12.495 glasova (48,09%) i 18 od 35 mandata.`,
      `Lista „Bor, naša odgovornost!" osvojila je 10.331 glas (39,76%) i 15 mandata.`,
      `Vlaška narodna stranka osvojila je 1.025 glasova (3,94%) i 2 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: „Irena Živković – Radnička – Bor Boranima" (2,27%), „Ljudi u centru – Srećko Zdravković" (1,6%), parlamentarna koalicija NADA (1,3%) i „Borci za Bor" (1,04%).`,
  },
  {
    name: "Aranđelovac",
    turnout: "25.999 od 36.841 birača (70,57%)",
    lists: [
      `Lista „Aleksandar Vučić – Aranđelovac, naša porodica!" osvojila je 13.030 glasova (50,12%) i 21 od 41 mandat.`,
      `Lista „Studenti za Aranđelovac – Mladost pobeđuje" osvojila je 11.979 glasova (46,08%) i 20 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: „Ruska stranka – Za bolji Aranđelovac" (1%), „Srpski liberali za zeleni Aranđelovac" (0,5%) i „Koalicija 381 – Ujedinjeni podržavamo mlade" (0,3%).`,
    note: "👉 Razlika: 1 mandat",
  },
  {
    name: "Bajina Bašta",
    turnout: "16.051 od 21.464 birača (74,78%)",
    lists: [
      `Lista „Aleksandar Vučić – Bajina Bašta, naša porodica!" osvojila je 8.415 glasova (52,42%) i 26 od 45 mandata.`,
      `Lista „Ujedinjeni za Bajinu Baštu" osvojila je 6.461 glas (40,25%) i 19 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: SDPS (2,27%), Zdrava Srbija (1,79%), „Nestranački pokret za Bajinu Baštu" (0,6%) i „Izađi – Izbori se za Bajinu Baštu – Srpski liberali" (0,4%).`,
  },
  {
    name: "Kladovo",
    turnout: "11.387 od 21.805 birača (54%)",
    lists: [
      `Lista „Aleksandar Vučić – Kladovo, naša porodica!" osvojila je 8.043 glasova (70,63%) i 21 od 28 mandata.`,
      `Lista „Kladovo Ima nas – Vreme je za promene" osvojila je 2.979 glasova (26,16%) i 7 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostala je lista „Srpski liberali za zeleno Kladovo" (1,2%).`,
  },
  {
    name: "Knjaževac",
    turnout: "15.883 od 22.262 birača (71,34%)",
    lists: [
      `Lista „Aleksandar Vučić – Knjaževac, naša porodica!" osvojila je 8.825 glasova (55,73%) i 24 od 40 mandata.`,
      `Lista „Knjaževac uz studente – Čista lista – Dr Ivan Milošević" osvojila je 5.102 glasa (32,22%) i 13 mandata.`,
      `Lista „Promene – Dr Igor Milosavljević – Marko Ignjatović" osvojila je 1.367 glasova (8,63%) i 3 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostala je lista „Srpski liberali za zeleni Knjaževac" (1%).`,
  },
  {
    name: "Kula",
    turnout: "23.310 od 33.286 birača (70%)",
    lists: [
      `Lista „Aleksandar Vučić – Kula, naša porodica!" osvojila je 11.438 glasova (49,06%) i 19 od 37 mandata.`,
      `Lista „Glas mladih opštine Kula" osvojila je 11.076 glasova (47,5%) i 18 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: „Mladi za Kulu" (0,5%), „Srpski liberali za zelenu Kulu" (0,3%) i Rusinska lista (0,2%).`,
    note: "👉 Razlika: 1 mandat",
  },
  {
    name: "Lučani",
    turnout: "11.562 od 14.631 birača (79,02%)",
    lists: [
      `Lista „Aleksandar Vučić – Lučani, naša porodica!" osvojila je 6.972 glasova (60,33%) i 23 od 35 mandata.`,
      `Lista „Zvuk pravde – Zajedno za studente – Mirjana Kern" osvojila je 3.803 glasova (32,9%) i 12 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: SDPS (2,1%), NADA (1,4%), BUNT (0,7%) i „Zeleni front za studente – Dragačevo" (0,5%).`,
  },
  {
    name: "Majdanpek",
    turnout: "9.609 od 15.441 birača (62,22%)",
    lists: [
      `Lista „Aleksandar Vučić – Majdanpek, naša porodica!" osvojila je 6.079 glasova (63,26%) i 21 od 31 mandat.`,
      `Lista „Nu dau" osvojila je 1.816 glasova (18,89%) i 6 mandata.`,
      `Lista „Mi odlučujemo! – Dr Damjan Stevkić" osvojila je 1.203 glasova (12,5%) i 4 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostala je lista „Srpski liberali za zeleni Majdanpek" (1,6%).`,
  },
  {
    name: "Smederevska Palanka",
    turnout: "25.605 od 38.933 birača (65,76%)",
    lists: [
      `Lista „Aleksandar Vučić – Smederevska Palanka, naša porodica!" osvojila je 14.161 glasova (56,57%) i 30 od 49 mandata.`,
      `Lista „Mladi za Palanku – Sami protiv svih" osvojila je 7.151 glas (28,5%) i 15 mandata.`,
      `Lista „Ujedinjeni za Palanku" osvojila je 2.214 glasova (8,8%) i 4 mandata.`,
    ],
    belowThreshold: `Ispod cenzusa ostale su liste: Studenti Palanke (1,7%) i „Izlečimo sistem – Da Palanka ozdravi" (0,8%).`,
  },
  {
    name: "Sevojno",
    turnout: "4.250 od 5.545 birača (76,64%)",
    lists: [
      `Lista „Aleksandar Vučić – Sevojno, naša porodica!" osvojila je 2.108 glasova (49,6%) i 9 od 19 mandata.`,
      `Lista „Ujedinjeni za Sevojno" osvojila je 1.906 glasova (44,84%) i 9 mandata.`,
      `Zdrava Srbija osvojila je 3,08% i 1 mandat.`,
    ],
    belowThreshold: `Ispod cenzusa ostala je lista „Srpski liberali za zeleno Sevojno" (0,7%).`,
    note: "👉 Odnos: 9–9 + 1 mandat odlučuje",
  },
];

export default function IzboriRezultati() {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  return (
    <div
      className="min-h-screen flex flex-col transition-colors duration-300"
      style={{ backgroundColor: isDark ? "#111318" : "#ffffff" }}
    >
      <SeoMeta
        path="/srbija/izbori-rezultati"
        title={TITLE}
        description={DECK}
        imageSrc={IMAGE_SRC}
      />
      <Header />

      <main
        className="py-12 md:py-16 flex-1"
        style={{ backgroundColor: isDark ? "#111318" : "#ffffff" }}
      >
        <article className="max-w-[860px] mx-auto px-5">
          {/* Rubrika */}
          <span className="kicker">Srbija</span>

          {/* Natpis */}
          <p
            className="mt-2 text-[13px] font-bold uppercase tracking-[0.18em]"
            style={{
              fontFamily: "'Source Sans 3', sans-serif",
              color: isDark ? "#d9bf7a" : "#8B0000",
            }}
          >
            🇷🇸 Izbori u Srbiji
          </p>

          {/* Naslov */}
          <h1
            className="mt-3 text-[30px] md:text-[44px] font-bold leading-[1.1]"
            style={{
              fontFamily: "'Playfair Display', serif",
              color: isDark ? "#e0ddd5" : "#111",
            }}
          >
            {TITLE}
          </h1>

          {/* Datum */}
          <p
            className="mt-3 uppercase"
            style={{
              fontFamily: "'Source Sans 3', sans-serif",
              fontSize: "12px",
              letterSpacing: "0.16em",
              color: isDark ? "#7f7a72" : "#888",
            }}
          >
            30. MART 2026.
          </p>

          {/* Podeli vest */}
          <ShareButton />

          {/* Uvodni paragraf */}
          <p
            className="mt-6 text-[18px] md:text-[20px] leading-[1.75]"
            style={{
              fontFamily: "'Lora', serif",
              color: isDark ? "#cfcac0" : "#222",
            }}
          >
            {DECK}
          </p>

          {/* Slika */}
          <div className="mt-8">
            <div
              className="border overflow-hidden"
              style={{
                borderColor: isDark ? "#2a2a2e" : "#eee",
                backgroundColor: isDark ? "#1a1c22" : "#f7f7f7",
              }}
            >
              <img
                src={IMAGE_SRC}
                alt="Ilustracija izbornih rezultata u Srbiji"
                className="w-full h-[260px] md:h-[420px] object-cover object-center block"
                decoding="async"
              />
            </div>
            <ImageCaption credit="Foto: ilustracija" />
          </div>

          {/* Rezultati po opštinama */}
          <div className="mt-8">
            {MUNICIPALITIES.map((m, idx) => (
              <div key={idx}>
                {/* Razdvajač */}
                <div
                  style={{
                    borderTop: `1px solid ${isDark ? "#2a2a2e" : "#e5e5e5"}`,
                    margin: "28px 0 20px",
                  }}
                />

                {/* Naziv opštine */}
                <p
                  className="text-[15px] font-bold uppercase tracking-[0.14em]"
                  style={{
                    fontFamily: "'Source Sans 3', sans-serif",
                    color: isDark ? "#d9bf7a" : "#8B0000",
                  }}
                >
                  📍 {m.name}
                </p>

                {/* Izlaznost */}
                <p
                  className="mt-2 text-[16px] leading-[1.7]"
                  style={{
                    fontFamily: "'Lora', serif",
                    color: isDark ? "#9a978f" : "#555",
                  }}
                >
                  <strong>Izlaznost:</strong> {m.turnout}
                </p>

                {/* Liste */}
                <ul className="mt-3 list-none p-0 m-0 space-y-2">
                  {m.lists.map((line, li) => (
                    <li
                      key={li}
                      className="text-[17px] md:text-[18px] leading-[1.75]"
                      style={{
                        fontFamily: "'Lora', serif",
                        color: isDark ? "#b7b2aa" : "#333",
                      }}
                    >
                      {line}
                    </li>
                  ))}
                </ul>

                {/* Ispod cenzusa */}
                <p
                  className="mt-3 text-[15px] leading-[1.7]"
                  style={{
                    fontFamily: "'Lora', serif",
                    color: isDark ? "#7a7872" : "#666",
                  }}
                >
                  {m.belowThreshold}
                </p>

                {/* Napomena (Razlika / Odnos) */}
                {m.note && (
                  <p
                    className="mt-3 text-[16px] font-semibold leading-[1.5]"
                    style={{
                      fontFamily: "'Lora', serif",
                      color: isDark ? "#d9bf7a" : "#8B0000",
                    }}
                  >
                    {m.note}
                  </p>
                )}
              </div>
            ))}
          </div>

          {/* Nazad */}
          <div className="mt-12">
            <a
              href="/srbija"
              className="inline-flex items-center gap-2 text-[13px] font-semibold tracking-[0.08em] uppercase no-underline"
              style={{
                fontFamily: "'Source Sans 3', sans-serif",
                color: isDark ? "#d9bf7a" : "#8B0000",
              }}
            >
              ← Nazad na Srbiju
            </a>
          </div>
        </article>
      </main>

      <Footer />
    </div>
  );
}
